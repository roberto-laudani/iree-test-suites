...
def main():
    ...
    print(f"  input  shape  {tuple(input_ids.shape)} {input_ids.dtype}")
    ...
    print(f"  output shape  {tuple(ref_logits.shape)} {ref_logits.dtype}")
    ...
    print(f"  output tokens {tokenizer.convert_ids_to_tokens(pred_ids)}")
    print(f"  next token    {tokenizer.decode([pred_ids[-1]])!r}")
